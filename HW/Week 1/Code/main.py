...
def train_using_gini(X_train, y_train):
    clf_gini = DecisionTreeClassifier(criterion='gini', random_state=6, max_depth=3, min_samples_leaf=5)
    clf_gini.fit(X_train, y_train)
    return clf_gini

def train_using_entropy(X_train, y_train):
    clf_entropy = DecisionTreeClassifier(criterion='entropy', random_state=6, max_depth=3, min_samples_leaf=5)
    clf_entropy.fit(X_train, y_train)
    return clf_entropy

def evaluation(clf, X_test, y_test):
    y_pred = clf.predict(X_test)
    # y_pred is a numpy array; show first few elements
    print(f'Results:\n{y_pred[:10]}')
